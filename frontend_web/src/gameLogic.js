...
/**
 * Clamp a value into an inclusive [min, max] range.
 *
 * @param {number} value
 * @param {number} min
 * @param {number} max
...
/**
 * Convert milliseconds to "m:ss" for UI clock display.
 *
 * @param {number} ms
 * @returns {string}
 */
...
/**
 * Normalize per-player time controls (in seconds) for the chess-clock mode.
 *
 * @param {number} seconds
 * @returns {number} clamped seconds in [5, 600]
 */
export function normalizeTimeControlSeconds(seconds) {
    const raw = Number(seconds);
    const floored = Number.isFinite(raw) ? Math.floor(raw) : 60;
    return clampNumber(floored, 5, 600);
}

/**
 * Determine whether a player has run out of time.
 *
 * @param {number} remainingMs
 * @returns {boolean}
 */
export function hasTimeExpired(remainingMs) {
    return Number(remainingMs) <= 0;
}
