...
function setTurnModeToTimed() {
    fireEvent.change(screen.getByLabelText(/Select turn mode/i), {
        target: { value: 'Timed (chess clock)' },
    });
}
...
describe('Game UI interactions', () => {
    ...
    it('timed turns mode shows clocks and counts down for the active player', () => {
        jest.useFakeTimers();
        ...
        // Ensure clock UI is visible.
        expect(screen.getByLabelText(/X time:/i)).toBeTruthy();
        expect(screen.getByLabelText(/O time:/i)).toBeTruthy();

        // Select 10s clock and verify initial label shows 0:10.
        fireEvent.change(screen.getByLabelText(/Select time per player/i), {
            target: { value: '10' },
        });
        expect(screen.getByLabelText(/X time: 0:10/i)).toBeTruthy();
        expect(screen.getByLabelText(/O time: 0:10/i)).toBeTruthy();

        // X is active; advance time and expect X to have decreased.
        act(() => {
            jest.advanceTimersByTime(1200);
        });
        expect(screen.getByLabelText(/X time: 0:0(8|9)/i)).toBeTruthy();
        expect(screen.getByLabelText(/O time: 0:10/i)).toBeTruthy();

        // Make X move; it becomes O's turn, then O should start decreasing.
        clickSquare('r1c1');
        act(() => {
            jest.advanceTimersByTime(1200);
        });

        expect(screen.getByLabelText(/O time: 0:0(8|9)/i)).toBeTruthy();

        jest.useRealTimers();
    });

    it('timed turns mode: player loses if their clock runs out', () => {
        jest.useFakeTimers();
        ...
        // Set a short clock (10s), then let X run out without moving.
        fireEvent.change(screen.getByLabelText(/Select time per player/i), {
            target: { value: '10' },
        });
        ...
        expect(screen.getByText(/Time out — Winner: O/i)).toBeTruthy();

        // Ensure no further moves can be made.
        expect(screen.getByLabelText(/Square r1c1, empty/i)).toBeTruthy();
        fireEvent.click(screen.getByLabelText(/Square r1c1, empty/i));
        expect(screen.getByLabelText(/Square r1c1, empty/i)).toBeTruthy();
